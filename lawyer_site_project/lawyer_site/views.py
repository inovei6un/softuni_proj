...
class AppointmentView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            time_range = datetime_obj - timedelta(minutes=59), datetime_obj + timedelta(minutes=59)
            stringified_range = f"{datetime_obj.strftime('%Y.%m.%d')}: {datetime_obj.strftime('%H:%M')} - {(datetime_obj + timedelta(minutes=59)).strftime('%H:%M')}"
            ...
            try:
                ...
                send_mail('Заявихте среща с адв. Рачева',
                          f"Данни на срещата: \n\n Дата и час: {stringified_range}\n Адрес: ул. Пейо Яворов №3, гр. Левски", 'advokat.r.racheva@gmail.com', [email])
